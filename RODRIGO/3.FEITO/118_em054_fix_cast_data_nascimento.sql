-- RAQ-MAND-EM054 — FIX: cast correto de valor->coluna na RPC de processar resposta
-- Bug: 'column "data_nascimento" is of type date but expression is of type text'
-- A RPC montava UPDATE dinamico com valor texto; agora casta para o tipo real da coluna.
-- CREATE OR REPLACE re-cria a funcao ja aplicada (mig 116) com a correcao.
    ...
    IF v_form.dedup_acao = 'mesclar' THEN
      ...
      LOOP
        ...
        IF v_col = ANY(v_contact_col_whitelist) THEN
          ...
          -- Merge NÃO-DESTRUTIVO: mantém o valor existente; só preenche se vazio/NULL.
          EXECUTE format(
            'UPDATE public.contacts SET %I = COALESCE(NULLIF(trim(%I::text), ''''), NULLIF(trim($1), ''''))::%s, updated_at = now() WHERE id = $2',
            v_col, v_col, v_coltype
